...
function readConfig(options) {
    ...
    */
    ...
    return new Promise((resolve, reject) => {
        ...
        if (!doc.my_organizations || !Array.isArray(doc.my_organizations)) {
            doc.my_organizations = [];
            console.warn(wrap("WARNING: you have no organizations configured " +
                "in config.yaml at all. This should be present, because otherwise " +
                "there's no real meaning to separating dashboards for 'inside' " +
                "and 'outside' the organization. Since you haven't defined one, these " +
                "dashboards will be the same until you do.", {width: 65}));
        }
